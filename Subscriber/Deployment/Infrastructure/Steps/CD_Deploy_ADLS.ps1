...
if($env:EnvOpts_CD_Services_Storage_ADLS_Enable -eq "True")
{
    ...
    az deployment group create -g $env:EnvOpts_CD_ResourceGroup_Name -f ././Templates/storageAccount.bicep --parameters location=$env:EnvOpts_CD_ResourceGroup_Location storageAccountName=$storageaccountname storageSKU=$env:EnvOpts_CD_Services_Storage_ADLS_StorageSKU
