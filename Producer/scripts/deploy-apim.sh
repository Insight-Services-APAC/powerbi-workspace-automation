...
ROOT_FILE_URL="https://$(storage).blob.core.windows.net/$CONTAINER/$REMOTE_TEMPLATE_PATH"
...
sed -i "s/test-syd-dev-fun-powerbi/$(func)/g" LOCAL_TEMPLATE_DIRECTORY/policies/*.xml

$FUNC_KEY = $(az functionapp keys list --name $(func) `
    --resource-group $(rg) `
...
az deployment group create \
  ...
  --parameters "FunctionBackendName=$(func)" \
  --verbose